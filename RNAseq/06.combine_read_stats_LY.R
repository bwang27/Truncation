...
input_readnum_f<- ifelse(is.na(args_v["input_readnum_f"]), paste(mapping_data_root,"/Input.read.num.log.out",sep=""), args_v["input_readnum_f"]) 
...
for(copy_out_folder in names(copy_f_list)){
	...
	for(file_var_name in names(file_list1)){
		...
		if(grepl("/$",file_path)){ #copy a folder
			cmd=paste("cp -f ", file_path,"*  \'", CopyOut_dir, "\'", sep=""); print(cmd)
			system(cmd)
			...
			if(file_path != ""){
				...
				if(file.exists(file_path)){
					if_file_exist=T
					system(paste("cp -f \'", file_path,"\'  \'", CopyOut_dir, "\'", sep=""))
				}
			}
...
system( paste("cp -f RNAseq_report.Rmd ",report_out_dir,  sep="") )
save.image(out_data_img_f)
