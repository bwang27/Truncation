...
#6/28/2022 added function to output mapped reads in fastq format 
#7/14/2022 when output mapped reads in fastq format, modified code to output the original strand sequence for reads mapped to "-" strand in a bam file (minimap2 will store the reverse-complementary strand sequence of a read, if it maps to the - strand. Have not checked other mappers)
...
if ($sample_name_str=$args{s}){ #sample name string
	...
	$out_mapped_reads_fq_f=$args{M} if $args{M} ne "";
...
foreach $samplename(@sample_names_arr){
	...
	if($out_mapped_reads_fq_f){
		create_dir_ifNotExist("$out_mapped_reads_fq_f");
		open(OUT_MAPPED_FQ_F,">$out_mapped_reads_fq_f") || die "error write $out_mapped_reads_fq_f\n";
	}
	...
	while(<SAM_F>){
		...
		if ($rname=~/\*/){#unmapped
			...
			if($out_mapped_reads_fq_f){ #this applied to single-end read only!
				if($cigar!~/H/ && !$AllMappedReadsId{$qname}){ #avoid duplicated output of a same reads with multiple mapping
					if($reads_strand eq "-"){
						$seq=&reverse_complement($seq);
						$qual=&reverse_str($qual);
					}
					print OUT_MAPPED_FQ_F '@'."$qname\n$seq\n+\n$qual\n";
					$AllMappedReadsId{$qname}=1;
				}
			}
		}
	...
	close OUT_MAPPED_FQ_F if $out_mapped_reads_fq_f;
