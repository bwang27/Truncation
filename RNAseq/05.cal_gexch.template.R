...
if(any(what2do %in% c("all","formatGeneTb")) ){
	...
	tb=as.matrix(apply(out_d[!if_exclude,gchtp_names, drop = FALSE],2,function(v){table( factor(v,levels=gexch_types) )})[gexch_types,], nrow=length(gexch_types))
	...
	tmp_tb=t(matrix(unlist(myApply(1:length(samplePairs), function(i){
		...
		if(any(if_up)){
			...
			return_v[5]=paste(paste(1:nrow(up_list), up_list[,1], round(up_list[,l2gch_names[i]],1), sep="|"), collapse=" ")
		}
		if(any(if_dn)){
			...
			return_v[6]=paste(paste(1:nrow(dn_list), dn_list[,1], round(dn_list[,l2gch_names[i]],1), sep="|"), collapse=" ")
		}
		return(return_v)
	})), nrow=6))
	topGeneList_tb[,c("max_absSS","max_absL2FC",paste0("List_",gexch_types[1:2]), paste0("ListDetail_",gexch_types[1:2]) )] = tmp_tb
