...
for(i in report_sample_is){
  ...
  if(isTRUE(max(chimericEvent_d1$count)>1)==TRUE ){
...
```{r MutationAnalysis, echo=FALSE,message=FALSE, results="asis", out.width='100%', collapse=FALSE, fig.cap=report_section_i}
cat(paste0("\n### Section ",report_section_i,": Mutation analysis: "))

sub_section=0
for(mutation_vcf in mutation_vcf[[out_sample]]){
  vector_name=gsub(".filtered.snps.vcf.gz","",mutation_vcf)
  sub_section=sub_section+1
  cat('<br><br>')
  cat( paste0('![](mutation_analysis/',out_sample,"/",mutation_vcf,')') )
  cat(paste0("\n\n**Figure ",report_section_i,".",sub_section,"  vector name and region: ", vector_name,"**\n\n"))
}
report_section_i=report_section_i+1
