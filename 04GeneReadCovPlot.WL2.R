...
#2022/10/04 WL: added code to also plot the GOI-plasmid homologous regions (defined by blat analysis) in other references as gene features
...
archPlot_min_read_count=ifelse(is.na(args_v["archPlot_min_read_count"]), 1, as.numeric(args_v["archPlot_min_read_count"]))
archPlot_max_ArchCount=500
...
read_bed_f<-function(bed_f, plasmid){ #read gene feature bed file and return multiple parameters
	bed_d=read.table(bed_f, header=F, sep="\t", quote="", stringsAsFactors=F)
	names(bed_d)=c("chromosome","start","end","name","score","strand")
	bed_d=bed_d[bed_d$chromosome %in% plasmid, ]
	if_GOI=grepl("GOI", bed_d[,4])
	GOI_coors=NA
	if(any(if_GOI)){
		bed_d[if_GOI,4]=paste0(bed_d[if_GOI,4],"(",bed_d[if_GOI,2]+1,"~",bed_d[if_GOI,3],", ", bed_d[if_GOI,3]-bed_d[if_GOI,2],"nt)")
		GOI_coors=c(bed_d[if_GOI,2][1]+1,  bed_d[if_GOI,3][1]) ##GOI start and end
	}
	feature_num=nrow(bed_d)
	return(list(bed_d=bed_d, if_GOI=if_GOI, GOI_coors=GOI_coors, feature_num=feature_num) )
}
...
sample_info_d$GOIblat_bed_f=paste0(index_folder,"/",sample_info_d$index_folder,"/", sample_info_d$index_folder,".fa.GOI.blat.bed")
...
#for each plasmid, find all samples used that vector and plot genome coverage using sashimi-plot.py (OLD way to plot using sashimi-plot.py)
...
for (plasmid_type in c("pHelper","pRepCap","GOI","Ad5")){
	...
	for(plasmid1 in plasmids){
		...
		bed_res_list=read_bed_f(bed_f, plasmid=plasmid1)
		bed_d=bed_res_list$bed_d
		if_GOI=bed_res_list$if_GOI
		GOI_coors=bed_res_list$GOI_coors
		feature_num=bed_res_list$feature_num
		...
		for(s in names(plot_sample_l)){
			...
			if(s !="All" & length(plot_sample_l[["All"]])==1){
				...
				for(rowi in 1:nrow(sample_info_d1)){
					...
					if(any(if_GOI)){
						annoHighlight(plot = signal1plot, chrom=plasmid1, chromstart=GOI_coors[1], chromend=GOI_coors[2], fill=NA, linecolor="gray",alpha=1,
							y = page_top_bottom_margin+(signalPlot_height+textPlot_height*2)*(rowi-1) + textPlot_height, height=signalPlot_height, lty=2)
					}
...
for (plasmid_type in c("pHelper","pRepCap","GOI","Ad5") ){
	...
	for(plasmid1 in plasmids){
		...
		for(rowi in 1:nrow(sample_info_d_pla1)){
			...
			mkdir_if_not_exist(out_prefix)
			addi_bed_f=sample_info_d_pla1$GOIblat_bed_f[rowi] #bed file with info of regions with homology to GOI plasmid
			bed_res_list2=read_bed_f(addi_bed_f, plasmid=plasmid1)
			if(nrow(bed_res_list2$bed_d)>0){
				bed_d=rbind(bed_d0, bed_res_list2$bed_d)
				feature_num=nrow(bed_d)
			}else{
				bed_d=bed_d0
			}
			...
			pageCreate(width = archPlot_width, height = plot_total_height, default.units = "inches", showGuides=F)
				...
				if(any(if_GOI)){
					annoHighlight(plot = signal1plot, chrom=plasmid1, chromstart=GOI_coors[1], chromend=GOI_coors[2], fill=NA, linecolor="gray",alpha=1,
						y = page_top_bottom_margin+ textPlot_height, height=signalPlot_height, lty=2)
				}
				...
				for(chimeric_typei in 1:length(chimeric_types)){
					...
					for(filei in 1:2){
						...
						bedpe_d=bedpe_d[bedpe_d[,8]>=archPlot_min_read_count, ]
						if(nrow(bedpe_d)>archPlot_max_ArchCount){bedpe_d=bedpe_d[1:archPlot_max_ArchCount, ]}
