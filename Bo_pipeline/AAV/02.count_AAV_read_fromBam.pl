...
require "../../../Pipeline/SharedCodes/perl.fun.inc.BW.pl";

getopt("nsbpouBtmcvSg",\%args);
...
$clip_cut=$args{c} ne ""? $args{c}: "0.4"; #maximal number of soft clip
$pairend_gapmax="6000"; # 
...
while(<SAM_F>){
	...
	# next if (($flag & 0x1) && (!$isize || abs($isize)>=$pairend_gapmax));
	# $counts{$sample}{"3.reads.$read_map_primary_orNot.isize<$pairend_gapmax"}++;
	...
	my $clip_nt=judge_clip_cutoff($cigar,$clip_cut);
	...
	##save chimeric reads information
