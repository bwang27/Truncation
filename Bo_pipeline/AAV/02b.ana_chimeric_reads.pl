...
$read_block_distance_max=5; #maximal distance between the read end coordinates for block1 and read start coordinates for block 2
...
$min_mapq_cutoff=$args{Q}?$args{Q}:50; #Minimal MAPQ score of the two alignments
$max_percMM_cutoff=$args{M}?$args{M}:0.15; #maximal percent mismatch of the two alignments
...
$ref_sizes_f=$args{S}? $args{S} : ""; #$index_folder/$index_name/$index_name.fa.sizes
	...
	if(/^ReadID|ChimericMappingInfo/){ #ReadID	ChimericMappingInfo
		...
		foreach my $i( 0..($mapping_block_num-2)){
			foreach my $j( ($i+1)..($mapping_block_num-1) ){
				...
				next if ($refName1 eq $refName2) && ($strand1 eq $strand2) && abs($refStart2-$refEnd1)<=$read_block_distance_max; ##may be short indels
				my $twoBlockDist=-1;
...
##output %chimericInfo2counts_h to out_chimericRead_count_f; create  %outBEDPE_row_h, %outbedrows_h;
my %outbedrows_h=();
my %outBEDPE_row_h=();
...
print OUT_CHIMERIC_COUNTS "chimeric_Ref_type	chimeric_type	refName1	refName2	strand1	strand2	refEnd1	refStart2	count\n";
foreach my $chimeric_key (sort { $chimericInfo2counts_h{$b} <=> $chimericInfo2counts_h{$a} } keys %chimericInfo2counts_h){
	...
	print OUT_CHIMERIC_COUNTS "$chimeric_key	$read_count\n";
