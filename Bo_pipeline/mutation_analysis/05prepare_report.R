...
mutation_dir=paste0(project_root, "/09_mutation/")
mutation_vcf=sapply(setdiff(list.dirs(mutation_dir,full.names=F),""), function(folder1){list.files(paste0(mutation_dir,folder1,"/"), ".snps.vcf.gz")}, simplify=F ); mutation_vcf

for(sample1 in names(mutation_vcf)){
        mkdir_if_not_exist(paste0(report_out_dir,"mutation_analysis/",sample1,"/"))
        for(f1 in mutation_vcf[[sample1]]){
                system( paste0("cp -f ", mutation_dir,sample1,"/",f1, " ", report_out_dir,"mutation_analysis/",sample1,"/",f1) )
        }
}
