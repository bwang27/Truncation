...
peakAnno_out_prefix=paste0(study_name, "/08_ClusterAnnotation/hostCellDNA_anno")
if(!is.na(args_v["peakAnno_out_prefix"])){ peakAnno_out_prefix= as.character(args_v["out_prefix"]) }
...
sample_info_f<- ifelse(is.na(args_v["sample_info_f"]), paste0("/home/Research_Archive/ProcessedData/Nanopore_seq/",study_name,"/", study_name,"-Samples.xlsx"), args_v["sample_info_f"]) 
...
system( paste0("cp NanoporeSeq_report.Rmd",  " ", report_out_dir,"NanoporeSeq_report.Rmd") )
...
if(file.exists(MinIONQC_sum_f)){
	...
	for(f1 in copy_files){
		system( paste0("cp ", MinIONQC_out_dir,f1, " ", report_out_dir,"MinIONQC/",f1) )
	}
}
...
mapping_stats_d=mapping_stats_d[order(match(mapping_stats_d[,1],samples)), ]
...
sampleConclusion_d=data.frame(Sample=report_samples
	...
	,ReadInfo=paste0("NGS generated ",formatC(mapping_stats_d$RawRead[report_sampleMap_ids], format="d", big.mark=",")," raw reads. After mapping, **", 
		apply(formatC(as.matrix(mapping_stats_d[report_sampleMap_ids, Used_read_headers, drop=F]),format="d", big.mark=","), 1, paste,sep=" and "), "**",
...
##5, read alignment coverage pictures
read_cov_dir=paste0(project_root, "/04_readCov_plot/")
read_cov_pic_fl=sapply(setdiff(list.dirs(read_cov_dir,full.names=F),""), function(folder1){list.files(paste0(read_cov_dir,folder1,"/"), ".png")}, simplify=F ); read_cov_pic_fl
read_cov_pic_fs=list.files(read_cov_dir, ".png", recursive=F); read_cov_pic_fs
...
for(sample1 in names(read_cov_pic_fl)){
	...
	for(f1 in read_cov_pic_fl[[sample1]]){
		system( paste0("cp ", read_cov_dir,sample1,"/",f1, " ", report_out_dir,"readCov_plot/",sample1,"/",f1) )
	}
}
...
for(f1 in readCovLen_pic_fs){
	system( paste0("cp ", project_root,"/03_ana_readMap/",f1, " ", report_out_dir,"readCovLen_boxplot/", f1) )
}
...
for(sample in AAV_samples){
	...
	system( paste0("cp ", peak_anno_f, " ", report_out_dir,"hostCellDNA_anno/") )
