...
#        if(($seperate_first[2]!~/\$seperate_element[2]/)&&($seperate_first[1]==$seperate_element[1])){
         ...
         if(($split_qry[2] ne $seperate_element[2])&&((($qryend[0]>$left_startread)&&($qryend[0]<$right_startread))||(($qrystart[0]>$leftread)&&($qrystart[0]<$rightread)))&&(($qrystart[1]>$left_startread_ir)&&($qrystart[1]<$right_startread_ir))){
   #   print SENSE "$_\n";}
