...
study_name=2022-09-21_AAV_SEQ
samples=(Packgene-MECP2-7-2 Packgene-MECP2-Mini Packgene-RP2 SS-AAV9-GTX-UbC-UBE3A-V5-02)
helpers=(pAdDeltaF6 pAdDeltaF6 pAdDeltaF6 pHelper)
RepCaps=(Cap9 Cap9 Cap8 pAAV9-GTX)
GOIs=(pscAAV.MeP426-MECP2_e1-7-2_shift pAAV-sc-MeP426-miniMECP2-V5-reg2-RDH1PA_shift pscAAV-RK-hybrid-intron-hRP2-BGHpA-KanR_WT_shift pAAV-UBE3A-KanR-3-shift1400)
GOIfolders=(MECP2 MECP2 RP2 UBE3A)
colors=(red pink purple green)
barcodes=(NBD04 NBD05 NBD06 NBD07)
if_multiple_GOI_in1batch=1
...
seq_sum_f=$seq_dir/sequencing_summary_FAU42352_b306ad47.txt #sequencing_summary_FAS00538_c41b1205.txt
...
##2, demultiplexing:
mkdir -p $mapping_data_root/fastq4/demultiplex
cd $mapping_data_root/fastq4/
...
for i in `seq 1 ${#barcodes[*]}`; do
  ...
  echo $barcodei_num
  mkdir -p $mapping_data_root/fastq4/demultiplex/barcode$barcodei_num/
  cd $mapping_data_root/fastq4/demultiplex/barcode$barcodei_num/
...
for i in `seq 1 ${#barcodes[*]}`; do
  ...
  barcodei_num=`echo $barcodei | sed -e 's/NBD//' -e 's/BC//'`
  mv $mapping_data_root/fastq4/demultiplex/barcode$barcodei_num/barcode$barcodei_num.fastq $sample1.fastq
done
...
cd /home/Research_Archive/ProcessedData/Nanopore_seq/$study_name/fastq4/renamed
...
if [ $if_multiple_GOI_in1batch -eq 1 ]; then
  ...
  mkdir minimap2
  fq_dir=$mapping_data_root/fastq4/renamed/ #the directory of the fastq files 
  ...
  for i in `seq 1 ${#samples[*]}`; do
    ...
    perl 01.Cal_geno_junc_readsnum_frSAM.pl -n $study_name -s "${samples[indx]}"  -d "$mapping_data_root/minimap2/" -o "$project_root/01.ReadTable/map2GOI/" \
      -e ".map2GOI.sam" -t minimap2 -q 0 -m 30 -u 0 -U $mapping_data_root/fastq4/${samples[indx]}.unmapped2GOI.fq -M $mapping_data_root/fastq4/${samples[indx]}.mapped2GOI.fq &
    p=$(($p+1)); echo $p; if [ $p -ge $nCPU ]; then  p=0 ;  wait; fi
  done
  wait
  ...
  cd $mapping_data_root
  mkdir minimap2
  fq_dir=$mapping_data_root/fastq4/ #the directory of the fastq files 
  ...
  for i in `seq 1 ${#samples[*]}`; do
    ...
    perl 01.Cal_geno_junc_readsnum_frSAM.pl -n $study_name -s "${samples[indx]}"  -d "$mapping_data_root/minimap2/" -o "$project_root/01.ReadTable/map2othGOI/" \
      -e ".map2otherGOI.sam" -t minimap2 -q 0 -m 30 -u 0 -U $mapping_data_root/fastq4/${samples[indx]}.unmapped2GOI.fq  &
    p=$(($p+1)); echo $p; if [ $p -ge $nCPU ]; then  p=0 ;  wait; fi
  done
  wait
  # 
...
rm *.map2GOI.bam
rm *.map2oths*
rm *.highQuality.bam #the final bam file will be $sample.sorted.bam
...
cd $AAVseqPipeline_root
/bin/Rscript /home/Research_Archive/ProcessedData/Nanopore_seq/2022-09-21_AAV_SEQ/05prepare_report.R -study_name $study_name -samples "${samples[*]}" -barcodes "${barcodes[*]}" -project_root $project_root \
